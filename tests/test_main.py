...
from src.main import Category, LawnGrass, Product, Smartphone, Order, load_data_from_json
...
def test_product_repr(sample_product: Product) -> None:
    ...
    assert repr(sample_product) == f"Product(name='Тестовый товар', price={sample_product.price}, quantity={sample_product.quantity})"


def test_product_str(sample_product: Product) -> None:
    """Тестирование строкового представления Product."""
    assert str(sample_product) == f"Тестовый товар, {sample_product.price} руб. Остаток: {sample_product.quantity} шт."
...
def test_product_addition_valid(sample_product: Product) -> None:
    ...
    assert total == (sample_product.price * sample_product.quantity + product2.price * product2.quantity)
...
def test_smartphone_repr(sample_smartphone: Smartphone) -> None:
    ...
    expected = (f"Smartphone(name='Тестовый смартфон', price={sample_smartphone.price}, quantity={sample_smartphone.quantity}, "
                f"efficiency='Высокая', model='Модель X', "
                f"memory='128GB', color='Черный')")
    assert repr(sample_smartphone) == expected


def test_smartphone_str(sample_smartphone: Smartphone) -> None:
    """Тестирование строкового представления Smartphone."""
    expected = (f"Тестовый смартфон, {sample_smartphone.price} руб. Остаток: {sample_smartphone.quantity} шт. "
...
def test_lawn_grass_repr(sample_lawn_grass: LawnGrass) -> None:
    ...
    expected = (f"LawnGrass(name='Тестовая трава', price={sample_lawn_grass.price}, quantity={sample_lawn_grass.quantity}, "
                f"country='Россия', germination_period='14 дней', "
                f"color='Зеленый')")
...
def test_lawn_grass_str(sample_lawn_grass: LawnGrass) -> None:
    """Тестирование строкового представления LawnGrass."""
    expected = (f"Тестовая трава, {sample_lawn_grass.price} руб. Остаток: {sample_lawn_grass.quantity} шт. "
